...
from .torch_egde import DenseDilatedKnnGraph, construct_hyperedges
...
class HypergraphConv2d(nn.Module):
    ...
    def __init__(self, in_channels=256, out_channels=256, act='relu', norm=None, bias=True):
        ...
        self.c = 512
        # self.c = 3
        self.c_means = nn.Parameter(torch.randn(256, self.c), requires_grad=True)
    ...
    def forward(self, x, hyperedge_matrix, point_hyperedge_index, centers):
        ...
        # CUDA_VISIBLE_DEVICES=5 python tools/train.py configs/dino/dino4.py
        # orin1 = orin.permute(0, 2, 1, 3)
        orin_out = torch.matmul(orin.squeeze(-1), self.c_means)
        print(orin_out.shape)
        ...
        aggregated_node_features_from_hyperedges = batched_index_select(aggregated_hyperedge_features.unsqueeze(-1),
                                                            point_hyperedge_index)

        out = (1 + self.eps) * orin + aggregated_node_features_from_hyperedges

        return orin_out
        # orin1 = orin.permute(0, 3, 2, 1)
        # orin_out = torch.matmul(orin1.squeeze(-1), self.c_means)
        # # print(orin_out.shape)
        # aggregated_hyperedge_features = node_features_for_hyperedges.sum(dim=-1, keepdim=True)
        #
        # # Step 2: Aggregate hyperedge features to update node features
        ...
        #                                                                 point_hyperedge_index)
        #
        # orin_out = (1 + self.eps) * orin_out + aggregated_node_features_from_hyperedges
        #
        # return orin_out
...
class GraphConv2d(nn.Module):
    ...
    #         return self.gconv(x, edge_index, y, hyperedge_matrix=hyperedge_matrix,
    #                           point_hyperedge_index=point_hyperedge_index, centers=centers)
    #     else:
    #         return self.gconv(x, edge_index, y)
    def forward(self, x, edge_index=None, y=None, hyperedge_matrix=None, point_hyperedge_index=None, centers=None):
        if isinstance(self.gconv, HypergraphConv2d):
            return self.gconv(x, hyperedge_matrix=hyperedge_matrix,
                              point_hyperedge_index=point_hyperedge_index, centers=centers)
        else:
            return self.gconv(x, edge_index, y)
...
class DyGraphConv2d(GraphConv2d):
    ...
    def forward(self, x, relative_pos=None):
        ...
        #     x = super(DyGraphConv2d, self).forward(x, hyperedge_matrix=hyperedge_matrix,
        #                                            point_hyperedge_index=point_hyperedge_index, centers=centers, y=y)
        if self.use_hypergraph:
            hyperedge_matrix, point_hyperedge_index, centers = construct_hyperedges(x, num_clusters=self.k)
            x = super(DyGraphConv2d, self).forward(x, hyperedge_matrix=hyperedge_matrix,
                                                   point_hyperedge_index=point_hyperedge_index, centers=centers)
...
class Grapher(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=9, dilation=1, conv='edge', act='relu', norm=None,
        ...
        # assert isinstance(in_channels, int), "in_channels must be an integer"s
    ...
    def _get_relative_pos(self, relative_pos, H, W):
        # if relative_pos is None or relative_pos.nelement() == 0 or H * W == self.n:
