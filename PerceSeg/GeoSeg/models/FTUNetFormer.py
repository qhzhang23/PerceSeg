...
from .graph.torch_vertex import Grapher
...
class RandomMaskingGenerator:
    ...
    def __call__(self):
        mask = np.hstack([
            np.zeros(self.num_patches - self.num_mask, dtype=np.float32),
            np.ones(self.num_mask, dtype=np.float32)
        ])
        np.random.shuffle(mask)
        # 将一维 Mask 转换为二维空间形状 (H', W')
        return mask.reshape(self.height, self.width)  # 新增 reshape 操作

    def __repr__(self):
        return f"Mask: total patches {self.num_patches}, masked {self.num_mask}"
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        # self.learnable_param1 = nn.Parameter(torch.ones(1, 256, 192, 192))
    ...
    def forward(self, res1, res2, res3, res4, h, w, train_mode):
        ...
        if y1.shape == (1, 256, 256, 256):
            x = x + self.learnable_param0 * y1
        # else:
        #     x = x + y1
        else:
            x = x + self.learnable_param1 * y1
...
class FTUNetFormer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.decoder(res1, res2, res3, res4, h, w, train_mode=self.training)
        return x
