...
class Lookahead(Optimizer):
    ...
    #     # 如果 self.optimizer 被 Scheduler wrap 了，就取出原始 optimizer
    #     raw_opt = getattr(self.optimizer, "optimizer", self.optimizer)
    #     # 真正调用 AdamW.step，而不是 wrapper
    #     loss = raw_opt.step(*args, **kwargs)
    #
    #     # 接着再做 Lookahead 的 slow/fast 更新
    #     for group in self.param_groups:
    #         if group["counter"] == 0:
    #             self.update(group)
