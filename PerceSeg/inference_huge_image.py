...
def loveda2rgb(mask): # LoveDA
    h, w = mask.shape[0], mask.shape[1]
    mask_rgb = np.zeros(shape=(h, w, 3), dtype=np.uint8)
    mask_convert = mask[np.newaxis, :, :]
    # water
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [255, 0, 0]
    # background
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [255, 255, 255]
    # building
    ...
    # barren
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [183, 129, 159]
    # forest
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [0, 255, 0]
    # agricultural
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [128, 195, 255]
...
def get_args():
    ...
    arg("-d", "--dataset", help="dataset", default="pv", choices=["pv", "landcoverai", "uavid", "building", "loveda"])
...
def main():
    ...
    for img_path in img_paths:
        ...
        if args.dataset == 'landcoverai':
            ...
            output_mask = loveda2rgb(output_mask)
